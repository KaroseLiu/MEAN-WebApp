...
angular.module('karose.directives', [])
  ...
  .directive('karoseDialog', ['$rootScope', 'modalDialogConfigureService', 'getDataService', 'KaroseCard', function($rootScope, modalDialogConfigureService, getDataService, KaroseCard){
		return {
      restrict: 'A',
      // scope: {
      // 	"cardData": "="
      ...
      link: function (scope, element, attrs) {
        ...
        function modalBtnClick(btn, pagesData){
            ...
            var params = pagesData[0].formData;
            getDataService.save(params, function(){
              $rootScope.$emit('dataChangeSuccessful');
              hideModal();
            });		
        	}else if(btn.operation === 'delete'){
            KaroseCard.deleteCard({id: $rootScope._id}, pagesData[0].formData, function(data){
              $rootScope.$emit('dataChangeSuccessful');
              hideModal();
            })
            ...
            KaroseCard.updateCard({id: $rootScope._id},pagesData[0].formData, function(data){
              $rootScope.$emit('dataChangeSuccessful');
              hideModal();
            })
